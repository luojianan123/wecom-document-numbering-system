...
def validate_node_code(
    ...
    if node.kind == "machine":
        pattern = rf"^GH{re.escape(project_code)}-[A-Z]{{1,6}}-00-[GZ]$"
        sequence = 0
    elif node.kind == "component":
        pattern = rf"^GH{re.escape(project_code)}-[A-Z]{{1,6}}-\d{{2}}-00-[GZ]$"
        sequence = int(parts[2]) if len(parts) == 5 and parts[2].isdigit() else -1
    elif node.kind in {"structure", "hardware"}:
        pattern = rf"^GH{re.escape(project_code)}-[A-Z]{{1,6}}-\d{{2}}-\d{{2}}-[GZ]$"
        sequence = int(parts[3]) if len(parts) == 5 and parts[3].isdigit() else -1
    elif node.kind == "part":
        pattern = rf"^GH{re.escape(project_code)}-[A-Z]{{1,6}}-\d{{2}}-\d{{2}}-\d{{2}}-[GZ]$"
        sequence = int(parts[4]) if len(parts) == 6 and parts[4].isdigit() else -1
    else:
...
def build_machine_code(project_code: str, name: str, stage: str) -> str:
    return f"GH{project_code}-{machine_abbreviation(name)}-00-{stage}"
...
def build_child_code(parent: ComponentNode, kind: str, sequence: int, stage: str) -> str:
    ...
    if kind == "component" and parent.kind == "machine":
        return "-".join([prefix[0], prefix[1], f"{sequence:02d}", "00", stage])
    if kind in {"structure", "hardware"} and parent.kind == "component":
        return "-".join([*prefix[:-1], f"{sequence:02d}", stage])
    if kind == "part" and parent.kind in {"structure", "hardware"}:
        return "-".join([*prefix, f"{sequence:02d}", stage])
...
def sequence_start(kind: str) -> int:
    return {"component": 1, "structure": 1, "hardware": 10, "part": 0}.get(kind, 0)
